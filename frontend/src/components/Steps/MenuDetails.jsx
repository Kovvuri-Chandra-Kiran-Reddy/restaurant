...
const MenuDetails = () => {
    ...
    const SubmitData = () => {
        if (!upload) {
            ...
            console.log("clickign submit : ", userData)
            setUpload(!upload)
            setStep(currentStep + 1)
        } else {

            var passwordToHash = userData.OwnerPassword;
            var hashedOwnerPassword = hasher(passwordToHash);
            ...
            console.log(userData.OwnerPassword)
    ...
    return (
        <div id='con'>
            <DisplayAllData />
            {!upload ? <><Button variant='contained' color='secondary' onClick={() => setFormVisibility(!FormVisible)} style={{ marginTop: "30px" }}> Add Items </Button><br /></> : ''}
